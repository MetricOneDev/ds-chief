...
contract DSChiefApprovals is DSThing {
    ...
    uint256 public constant threshold = 80_000 * 10 ** 18; // 80K GOV_TOKEN threshold
    ...
    {
        require(!live);
        require(hat == address(0) && approvals[address(0)] >= threshold, "DSChiefApprovals/not enough votes on 0x address");
        live = true;
    }
    ...
    function lift(address whom)
        ...
        note
    {
        require(approvals[whom] > threshold && approvals[whom] > approvals[hat], "DSChiefApprovals/not enough votes");
        hat = whom;
    }
